...
async function runDBConnection() {
    try {
        ...
        collection = client.db("CoffeeDB").collection("CoffeeCollection"); // Create a specific database and collection
...
app.post('/add-coffee', async (req, res) => {
    try {
        const data = req.body;
        await collection.insertOne(data); // Add form data to the database
...
app.get('/get-coffees', async (req, res) => {
    try {
        const data = await collection.find().toArray(); // Retrieve all data from the collection
